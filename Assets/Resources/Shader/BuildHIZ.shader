Shader "Hidden/BuildHIZ"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 offset = _CameraDepthTexture_TexelSize.xy * 0.25;
         ...
         Pass
        {
            ...
            TEXTURE2D_FLOAT(_HizDepthRT);
            SAMPLER(sampler_HizDepthRT);
            //float4 _HizDepthRT_TexelSize;
            ...
            float frag (Varyings i) : SV_Target
            {
                float2 offset = _SrcSize.xy * 0.25;

                float x = SAMPLE_TEXTURE2D_LOD(_HizDepthRT,sampler_HizDepthRT, i.uv + offset, _MipLevel).x;
                float y = SAMPLE_TEXTURE2D_LOD(_HizDepthRT,sampler_HizDepthRT, i.uv - offset, _MipLevel).x;
                float z = SAMPLE_TEXTURE2D_LOD(_HizDepthRT,sampler_HizDepthRT, i.uv + float2(offset.x, -offset.y), _MipLevel).x;
                float w = SAMPLE_TEXTURE2D_LOD(_HizDepthRT,sampler_HizDepthRT, i.uv + float2(-offset.x, offset.y), _MipLevel).x;
                float4 readDepth = float4(x,y,z,w);
                #if UNITY_REVERSED_Z
                    readDepth.xy = min(readDepth.xy, readDepth.zw);
                    readDepth.x = min(readDepth.x, readDepth.y);
         ...
         Pass
        {
            ...
            float frag (Varyings i) : SV_Target
            {
                float x = SAMPLE_TEXTURE2D_LOD(_HizDepthRT,sampler_HizDepthRT, i.uv, _MipLevel).x;
